...
export default function ReportDetail() {
  ...
  // Detect admin role from Firebase token claims.
  useEffect(() => {
    if (!user) return;
    user.getIdTokenResult().then((res) => setIsAdmin(res.claims.role === "admin"));
  }, [user]);

  // Load the report (and categories for the admin dropdown) whenever the id changes.
  useEffect(() => {
    load();
  ...
  async function changeCategory(newCategoryId) {
    setUpdating(true);
    try {
      await api.patch(`/reports/${id}/category`, { category_id: Number(newCategoryId) });
      await load();
    } catch (e) {
      alert(e.response?.data?.detail || e.message);
    } finally {
        ...
        {isAdmin && (
          <div className="mt-4 pt-4 border-t border-outline space-y-3">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {cluster && (
                <span className="text-xs text-muted">
                  Will update all {cluster.repeated_count} reports.
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <select
                value={report.category.category_id}
                onChange={(e) => changeCategory(e.target.value)}
                disabled={updating || categories.length === 0}
                className="text-sm border border-outline rounded px-2 py-1 focus:border-navy outline-none"
              >
                {categories.map((c) => (
                ...
                <div className="flex justify-between items-start gap-3">
                  <div>
                    <p className="text-xs text-muted">
                      #{m.report_id}
                      {m.report_id === report.report_id && (
                        <span className="ml-2 text-accent font-semibold">(this one)</span>
                      )}
                    </p>
          ...
          <ol className="space-y-3">
            {history.map((h, i) => (
              <li key={i} className="flex gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink">{h.status.replace("_", " ")}</p>
                  <p className="text-xs text-muted">{format(new Date(h.updated_at), "PPp")}</p>
